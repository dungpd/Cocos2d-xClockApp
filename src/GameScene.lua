...
--visibleSize.height = 800

local GameScene = class("GameScene",function()
    return cc.Scene:create()
end)

local ClockScene = class("CloackScene",function()
    return cc.Scene:create()
end)

function GameScene.create()
   local scene = ClockScene.new()
   scene:addChild(scene:createClockLayer())
...
function ClockScene:createClockLayer()
    local layer = cc.Layer:create()
    local time = os.date("*t")
    
    local clock = cc.Label:createWithSystemFont("", "Arial", 500)
--    local function reloadClock()
--       --cclog("test reloadclock")
--       time = os.date("*t")
--       clock.setString(time["hour"]..":"..time["min"]..":"..time["sec"])
--    end
--    self.schedulerID = cc.Director:getInstance():getScheduler():scheduleScriptFunc(reloadClock,0,false)
    --clock:setAnchorPoint(0.5,0.5)
    clock:setPosition(visibleSize.width/2,visibleSize.height/2)
    cclog("test setpos")
    ...
    local function reload()
        ...
        time = os.date("*t")
        if time["min"] < 10 then
            clock:setString(time["hour"]..":".."0"..time["min"])
        else
            clock:setString(time["hour"]..":"..time["min"])
        end    
    end

    self.schedulerID = cc.Director:getInstance():getScheduler():scheduleScriptFunc(reload, 0, false)
